...
function getVisibleQuestions(answers: Record<string, string>) {
...
export default function useFormState() {
	...
	const next = () => setCurrentIndex(i => Math.min(i + 1, visibleQuestions.length - 1));
	const back = () => setCurrentIndex(i => Math.max(i - 1, 0));

	return { currentQuestion, currentIndex, totalSteps, answers, storeAnswer, next, back }
}